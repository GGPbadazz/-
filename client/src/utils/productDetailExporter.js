...
export class ProductDetailExporter {
  ...
  createProductDetailSheet() {
    ...
    const statsRow = worksheet.addRow([
      `统计期间: ${this.period} | 条码: ${this.product.barcode || '-'} | 类别: ${this.product.category_name || '-'} | 当前库存: ${this.formatQuantity(this.product.current_stock)} | 库存单价: ¥${(this.product.current_unit_price || 0).toFixed(2)} | 库存价值: ¥${(this.product.total_cost_value || 0).toFixed(2)}`,
      '', '', '', '', '', '', '', '', '', '', ''
    ]);
    ...
    if (this.transactions.length > 0) {
      ...
      const summaryRow = worksheet.addRow([
        ...
        `入库金额: ¥${totalInboundValue.toFixed(2)}`,
        `出库金额: ¥${totalOutboundValue.toFixed(2)}`,
        '', '', '', ''
      ]);
