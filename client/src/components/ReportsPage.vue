...
export default {
  ...
  setup() {
    ...
    const exportInventoryExcel = () => {
      ...
      // 准备Excel数据 - 使用真实的产品价格数据
      const excelData = data.map(product => {
        // 使用 current_unit_price 或 price 作为库存单价
        const unitPrice = product.current_unit_price || product.price || 0;
        // 使用 total_cost_value 或计算得出的总价
        const totalValue = product.total_cost_value || (unitPrice * (product.stock || 0));
        ...
        return {
          ...
          '库存单价': parseFloat((unitPrice).toFixed(2)),
          '库存总价': parseFloat(totalValue.toFixed(2)),
