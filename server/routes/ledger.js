...
router.get('/monthly', (req, res) => {
    try {
        ...
        const getBeginningStock = () => {
            if (isCurrentMonth) {
                console.log('🔄 当前月份: 计算期初库存 = 当前库存 - 当月净变化');
                
                // 首先获取当月交易的净变化
                const monthlyChangeQuery = `
                    SELECT 
                        t.product_id,
                        SUM(CASE WHEN t.type = 'IN' THEN t.quantity ELSE -t.quantity END) as monthly_change,
                        SUM(CASE WHEN t.type = 'IN' THEN COALESCE(t.total_price, t.quantity * t.unit_price) 
                                 ELSE -COALESCE(t.total_price, t.quantity * t.unit_price) END) as monthly_value_change
                    FROM transactions t
                    WHERE t.created_at >= ? AND t.created_at < ?
                    GROUP BY t.product_id
                `;
                
                const monthlyChanges = db.prepare(monthlyChangeQuery).all(startDate, endDate);
                const monthlyChangeMap = new Map();
                monthlyChanges.forEach(row => {
                    monthlyChangeMap.set(row.product_id, {
                        monthly_change: row.monthly_change || 0,
                        monthly_value_change: row.monthly_value_change || 0
                    });
                });
                ...
                const allProductsQuery = `
                    SELECT 
                        ...
                        current_unit_price,
                        total_cost_value as current_stock_value
                    FROM products
                `;
                
                const allProducts = db.prepare(allProductsQuery).all();
                
                return allProducts.map(product => {
                    const monthlyChange = monthlyChangeMap.get(product.product_id) || { monthly_change: 0, monthly_value_change: 0 };
                    
                    // 期初库存 = 当前库存 - 当月净变化
                    const beginning_stock = Math.max(0, product.current_stock - monthlyChange.monthly_change);
                    const beginning_stock_value = Math.max(0, product.current_stock_value - monthlyChange.monthly_value_change);
                    const beginning_unit_price = beginning_stock > 0 ? beginning_stock_value / beginning_stock : product.current_unit_price;
                    
                    return {
                        product_id: product.product_id,
                        beginning_stock,
                        beginning_unit_price: Math.round(beginning_unit_price * 100) / 100,
                        beginning_stock_value: Math.round(beginning_stock_value * 100) / 100
                    };
                }).filter(row => row.beginning_stock > 0 || monthlyChangeMap.has(row.product_id)); // 包含有库存或有交易的产品
            } else {
                console.log('📸 历史月份: 使用快照数据获取期初库存');
                // 历史月份：优先使用上月快照
                const snapshotQuery = `
                    SELECT 
                        product_id,
                ...
                // 如果没有历史交易数据，使用产品表中的当前库存作为期初库存
                if (historicalResults.length === 0) {
                    console.log('📦 没有历史交易数据，使用产品表当前库存作为期初库存');
                    const currentStockQuery = `
                        SELECT 
                            id as product_id,
                            stock as beginning_stock,
                            current_unit_price as beginning_unit_price,
                            total_cost_value as beginning_stock_value
                        FROM products
                        WHERE stock > 0
                    `;
        ...
        beginningStockResults.forEach(row => {
            ...
            beginningStockMap.set(row.product_id, {
                ...
                beginning_unit_price: Math.round(beginningUnitPrice * 100) / 100,
                beginning_stock_value: Math.round(beginningStockValue * 100) / 100
            });
        });
        ...
        endingStockResults.forEach(row => {
            ...
            endingStockMap.set(row.product_id, {
                ...
                ending_unit_price: Math.round(endingUnitPrice * 100) / 100,
                ending_stock_value: Math.round(endingStockValue * 100) / 100
            });
        });
